...
export const analyzeBlogSEO = (post) => {
  ...
  if (seo.title && seo.title.trim()) {
    ...
    recommendations.push({ key: 'seoTitle', priority: 'high', text: 'Missing SEO title', fix: 'Add a concise, descriptive SEO title (30–70 characters).' });
  }
  if (seo.title && seo.title.trim().length >= 30 && seo.title.trim().length <= 70) {
    ...
    recommendations.push({ key: 'seoTitleLen', priority: 'low', text: 'SEO title length outside guidance', fix: 'Aim for 30–70 characters so it is not truncated in search results.' });
  }
  ...
  if (seo.description && seo.description.trim()) {
    ...
    recommendations.push({ key: 'metaDesc', priority: 'high', text: 'Missing meta description', fix: "Add a concise description explaining the article's purpose." });
  }
  if (seo.description && seo.description.trim().length >= 80 && seo.description.trim().length <= 160) {
    ...
    recommendations.push({ key: 'metaDescLen', priority: 'low', text: 'Meta description length outside guidance', fix: 'Aim for 80–160 characters.' });
  }
  ...
  if (!seo.noindex) {
    ...
    recommendations.push({ key: 'indexable', priority: 'medium', text: 'Article is set to NOINDEX', fix: 'Set Robots to "Index, Follow" unless you deliberately want this hidden from search.' });
  }
  ...
  if (post.slug && !/\s/.test(post.slug) && post.slug === post.slug.toLowerCase()) {
    ...
    recommendations.push({ key: 'slug', priority: 'low', text: 'Slug is not SEO-friendly', fix: 'Use a short, lowercase, hyphen-separated slug.' });
  }
  ...
  if (h1Count === 1) {
    ...
    recommendations.push({ key: 'h1', priority: 'medium', text: 'No H1 heading in the article', fix: 'Add a single H1 heading (usually the title) at the top of the article.' });
  } else {
    ...
    recommendations.push({ key: 'h1', priority: 'high', text: `${h1Count} H1 headings found`, fix: 'Keep only one H1 per article; demote extras to H2/H3.' });
  }
  ...
  if (wordCount >= 600) {
    ...
    recommendations.push({ key: 'depth', priority: 'medium', text: `Article is relatively short (${wordCount} words)`, fix: 'Expand with more detail, examples, or a dedicated section covering related sub-topics.' });
  } else {
    checks.push({ key: 'depth', status: 'bad', label: `Very thin content (${wordCount} words)` });
    recommendations.push({ key: 'depth', priority: 'high', text: `Very thin content (${wordCount} words)`, fix: 'Add substantially more useful, original content.' });
  }
  ...
  if (h2h3Count >= 3) {
    ...
    recommendations.push({ key: 'headings', priority: 'medium', text: 'Limited heading structure', fix: 'Break the article into more H2/H3 sections for scannability.' });
  } else {
    checks.push({ key: 'headings', status: 'bad', label: 'No H2/H3 headings found' });
    recommendations.push({ key: 'headings', priority: 'medium', text: 'No H2/H3 headings', fix: 'Add H2/H3 section headings to structure the article.' });
  }
  ...
  if (post.faqs && post.faqs.length > 0) {
    ...
    recommendations.push({ key: 'faqs', priority: 'low', text: 'No FAQ section', fix: 'Consider adding 3–5 frequently asked questions relevant to this topic.' });
  }
  ...
  if (internalLinks.length >= 2) {
    ...
    recommendations.push({ key: 'internalLinks', priority: 'medium', text: 'Only 1 internal link', fix: 'Link to 2–3 relevant Apex Vouchers pages (related vouchers, policies, or guides).' });
  } else {
    checks.push({ key: 'internalLinks', status: 'bad', label: 'No internal links' });
    recommendations.push({ key: 'internalLinks', priority: 'medium', text: 'No internal links found', fix: 'Add internal links to relevant Apex Vouchers pages using the Links tab.' });
  }
  ...
  if (focusKeyword) {
    ...
    if (kwHits >= 3) {
      ...
      recommendations.push({ key: 'keyword', priority: 'low', text: 'Primary keyword underused', fix: 'Mention the primary keyword naturally in the title, intro paragraph and meta description.' });
    }
  }
  ...
  if (post.coverImage) {
    ...
    recommendations.push({ key: 'coverImage', priority: 'medium', text: 'No featured image', fix: 'Upload a featured image — improves click-through from search and social shares.' });
  }
  ...
  if (post.coverImageAlt && post.coverImageAlt.trim()) {
    ...
    recommendations.push({ key: 'coverAlt', priority: 'low', text: 'Featured image missing ALT text', fix: 'Add a natural, descriptive ALT text for the featured image.' });
  }
  ...
  if (totalImages === 0) {
    ...
    recommendations.push({ key: 'imageAltCoverage', priority: 'low', text: `Add ALT text to ${missing} image${missing === 1 ? '' : 's'}`, fix: 'Describe each image naturally for accessibility and SEO.' });
  }
