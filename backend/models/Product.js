...
// ── Product-level guide CMS ─────────────────────────────────────────────────
// Each product carries its own complete "How to Purchase" and "How to Redeem"
// content so instructions are data-driven and product-specific — no hard-coded
// per-exam logic in the frontend. See web/lib/redemption-guides.ts /
// web/lib/purchase-guide.ts for the render-time fallback chain.

const guideScreenshotSchema = new mongoose.Schema({
...
const redemptionStepSchema = new mongoose.Schema({
  ...
  title: { type: String, trim: true, default: '' },
  description: { type: String, trim: true, default: '' },
  screenshot: { type: guideScreenshotSchema, default: () => ({}) },
...
const purchaseStepSchema = new mongoose.Schema({
  order: { type: Number, default: 0 },
  title: { type: String, trim: true, default: '' },
  description: { type: String, trim: true, default: '' },
  screenshot: { type: guideScreenshotSchema, default: () => ({}) },
...
const purchaseGuideSchema = new mongoose.Schema({
  enabled: { type: Boolean, default: false },
  eyebrow: { type: String, trim: true, default: '' },
  title: { type: String, trim: true, default: '' },
  description: { type: String, trim: true, default: '' },
  steps: { type: [purchaseStepSchema], default: [] },
  lastUpdated: { type: Date },
}, { _id: false });
...
const productSchema = new mongoose.Schema(
  {
    ...
    // Product-specific "How to Purchase" CMS.
    purchaseGuide: { type: purchaseGuideSchema, default: () => ({}) },
