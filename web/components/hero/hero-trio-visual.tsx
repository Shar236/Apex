...
import { LazyMotion, domAnimation, m, useReducedMotion, useMotionValue, useSpring, useTransform } from 'framer-motion';
import { useTheme } from '@/components/theme-provider';

/*
 * The single right-side hero visual — one theme-aware, Framer-Motion composition:
 *   layer 0  ambient light + soft background blobs   (parallax x0.35, slowest)
 *   layer 1  wireframe globe                          (parallax x0.5, slow spin)
 *   layer 2  the trio cut-out                         (parallax x1.0, gentle float)
 *   layer 3  floating voucher cards + specks          (parallax x1.45, fastest)
 * prefers-reduced-motion -> a clean static render (no float, spin or parallax).
 */

interface VoucherCardSpec {
  pos: string;
  delay: number;
  tile: string;
  initial: string;
...
const VOUCHER_CARDS: VoucherCardSpec[] = [
...
function FloatingVoucherCard({ pos, tile, initial, name, save, delay, reduced }: VoucherCardSpec & { reduced: boolean | null }) {
  return (
    <m.div
    ...
    >
      <div className="bg-surface/95 backdrop-blur-md p-3 sm:p-3.5 px-4 rounded-2xl border border-line shadow-xl dark:shadow-[0_16px_40px_-12px_rgba(0,0,0,0.7)] flex items-center gap-3">
        <div className={`w-10 h-10 rounded-2xl ${tile} font-medium text-lg flex items-center justify-center shrink-0 shadow-md`}>{initial}</div>
        <div className="flex flex-col text-left">
          ...
          <span className="mt-1 inline-flex items-center px-2.5 py-0.5 rounded-full bg-success text-white font-medium text-[10px] shadow-sm">SAVE {save}</span>
        </div>
      </div>
    </m.div>
  );
...
function WireGlobe({ isDark }: { isDark: boolean }) {
...
export function HeroTrioVisual() {
  ...
  const entrance = prefersReduced
    ...
    : {
        ...
        transition: { type: 'spring' as const, stiffness: 55, damping: 16, mass: 1 },
      };
  ...
  return (
    <LazyMotion features={domAnimation} strict>
      ...
      >
        <m.div className="absolute inset-0 pointer-events-none" style={parallaxOn ? { x: bgX, y: bgY } : undefined}>
          <div
            className={`absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full blur-3xl ${
              isDark ? 'w-[115%] h-[115%] bg-[radial-gradient(circle,rgba(255,0,92,0.22),rgba(93,63,211,0.12)_45%,transparent_72%)]' : 'w-[105%] h-[105%] bg-[radial-gradient(circle,rgba(255,0,92,0.14),transparent_68%)]'
            }`}
          />
        ...
        <m.div className="absolute left-1/2 top-[46%] -translate-x-1/2 -translate-y-1/2 w-[92%] h-[92%] pointer-events-none" style={parallaxOn ? { x: globeX, y: globeY } : undefined}>
          <m.div className="w-full h-full" animate={prefersReduced ? undefined : { rotate: 360 }} transition={prefersReduced ? undefined : { duration: 140, repeat: Infinity, ease: 'linear' }}>
            <WireGlobe isDark={isDark} />
          </m.div>
        </m.div>

        <m.div className="relative z-20 h-full flex items-end justify-center [transform-style:preserve-3d]" style={parallaxOn ? { x: trioX, y: trioY } : undefined}>
          <m.img
            src="/hero-trio.png"
        ...
        <m.div className="absolute inset-0 z-30 pointer-events-none" style={parallaxOn ? { x: cardsX, y: cardsY } : undefined}>
          {VOUCHER_CARDS.map((c) => (
          ...
          {!prefersReduced && (
            <>
              <m.span className="absolute top-[30%] left-[18%] w-2 h-2 rounded-full bg-accent/70" animate={{ y: [0, -10, 0], opacity: [0.4, 1, 0.4] }} transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut' }} />
              <m.span
