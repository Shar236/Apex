    ...
    <section className="mt-12 pt-10 border-t border-[#EAEAEA] dark:border-[#292929]" aria-label="Frequently Asked Questions">
      ...
      <div className="space-y-3">
        {faqs.map((f, idx) => {
          ...
          return (
            <div key={f.question} className={`faq-accordion rounded-2xl border transition-all ${isOpen ? 'border-brand-pink/40 bg-[#FFF0F5] dark:bg-[#2A0A17]' : 'border-[#EAEAEA] dark:border-[#292929] bg-white dark:bg-[#161616]'}`}>
              <button type="button" onClick={() => setOpen(isOpen ? null : idx)} aria-expanded={isOpen} className="w-full flex items-center justify-between gap-3 px-5 py-4 text-left cursor-pointer">
