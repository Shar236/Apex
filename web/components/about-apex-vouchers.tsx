...
export function AboutApexVouchers() {
  return (
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center mb-16">
          <div className="lg:col-span-6 space-y-6">
            <span className="inline-flex text-[11px] font-medium uppercase tracking-[0.14em] text-accent">Our Mission &amp; Story</span>
            ...
            <p className="text-ink-muted font-normal text-sm leading-relaxed">By partnering with authorized institutional channels and buying bulk exam vouchers directly, we pass maximum savings back to candidates without any compromise on validity or support.</p>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 pt-6 border-t border-line">
          {GENUINE_STATS.map((st) => (
            <div key={st.label} className="bg-surface rounded-2xl p-6 border border-line shadow-sm text-center space-y-1">
              <span className="font-heading font-normal text-3xl sm:text-4xl text-ink block">{st.number}</span>
              <span className="text-xs font-medium text-accent uppercase tracking-wider block">{st.label}</span>
              <span className="text-[11px] text-ink-muted font-normal block">{st.desc}</span>
            </div>
          ))}
        </div>
      </div>
