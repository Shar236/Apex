...
/** Six-box accessible OTP input. Auto-focus/advance, backspace navigation, full paste support. */
export function OtpInput({
  ...
  return (
    <div>
      <div className={`flex items-center justify-center gap-2 sm:gap-3 ${error ? 'animate-[shake_0.4s_ease-in-out]' : ''}`} role="group" aria-label={label}>
        {digits.map((digit, index) => (
          <input
            ...
            className={`w-10 h-12 sm:w-12 sm:h-14 text-center text-lg sm:text-xl font-medium rounded-2xl border-2 outline-none transition-colors bg-surface-raised text-ink disabled:opacity-50 ${
              error ? 'border-rose-500 focus:border-rose-500' : 'border-line focus:border-accent'
            }`}
          />
        ))}
      </div>
