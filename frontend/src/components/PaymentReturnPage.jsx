...
/**
 * Fallback payment-status page.
 *
 * Razorpay Checkout is modal-based, so most users never land here — the
 * CheckoutModal verifies the payment inline. This page exists for the edge
 * cases (browser closed mid-flow, deep link, manual refresh).
 *
 * SECURITY: this page ONLY reads the server's truth via GET /api/payments/order/:id.
 * It cannot and does not mark anything paid. A voucher shows here only because
 * the backend already verified the payment (inline or via the signed webhook).
 */
export const PaymentReturnPage = () => {
  const [searchParams] = useSearchParams();
  const orderId =
    searchParams.get('order_id') || searchParams.get('orderId') || searchParams.get('orderNo') || '';
  ...
  const check = useCallback(async () => {
    ...
    try {
      ...
      const paid =
        ...
        (res.orderStatus === 'FULFILLED' || res.fulfillmentStatus === 'FULFILLED');
  ...
  useEffect(() => {
    ...
    check().then((res) => {
      const settled =
        res &&
        (res.paymentStatus === 'PAID' || res.paymentStatus === 'FAILED');
      // Payment may still be settling via the webhook — retry a few times.
      if (alive && !settled) {
        const t = setInterval(() => {
          setAttempts((a) => {
            if (a >= 10) {
            ...
            check().then((r) => {
              if (r && (r.paymentStatus === 'PAID' || r.paymentStatus === 'FAILED')) {
                clearInterval(t);
              }
            });
            return a + 1;
        ...
        {loading ? (
          <div className="py-12 space-y-4">
            ...
            <p className="text-xs text-ink-muted font-normal">
              We&apos;re confirming your payment with Razorpay and preparing your voucher.
            </p>
          </div>
        ) : isPaid ? (
          <div className="py-4 space-y-6">
            ...
            {vouchers.length > 0 ? (
              <div className="space-y-3 text-left">
                {vouchers.map((v, i) => (
                  <div key={i} className="p-5 rounded-2xl bg-accent/8 border-2 border-dashed border-accent/40 space-y-2">
                    ...
                    <div className="font-mono font-medium text-2xl tracking-wider select-all break-all text-ink">
                      {v.code}
                    </div>
                  </div>
                ))}
              ...
              <div className="p-4 rounded-2xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 text-xs font-normal text-success">
          ...
          <div className="py-6 space-y-5">
            ...
            <div>
              <span className="text-xs font-medium uppercase tracking-widest text-amber-600 block mb-1">
                {isPending ? 'PAYMENT PENDING' : 'PAYMENT NOT CONFIRMED'}
              </span>
              <h2 className="font-heading font-medium text-2xl">
                {isPending ? 'Still confirming…' : 'Payment status'}
              </h2>
              <p className="text-xs text-ink-muted font-normal mt-1.5 max-w-sm mx-auto">
                {error
                  ? error
            ...
            <div className="flex gap-3 pt-2">
              <button
                ...
                className="flex-1 py-3.5 rounded-2xl bg-surface-raised font-medium text-xs border border-line"
              >
                Refresh status
              </button>
              <Link
                to="/account"
                className="flex-1 py-3.5 rounded-2xl bg-accent hover:bg-accent-hover text-white font-medium text-xs transition-colors flex items-center justify-center"
              >
                Candidate Vault
              </Link>
            </div>
