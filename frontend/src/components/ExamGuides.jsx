...
const BlogAwardsSection = () => {
  ...
  return (
    <div className="mt-16 sm:mt-24 pt-16 sm:pt-20 border-t border-line">
      {/* Section Header */}
      <SectionHeading
    ...
    <section className="py-16 sm:py-24 bg-surface border-b border-line transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex justify-center -mt-4 mb-12">
          <Link
            to="/blog"
            className="inline-flex items-center gap-1.5 px-4 h-10 rounded-full bg-surface text-accent font-medium text-xs border border-accent/45 hover:bg-accent/6 transition-colors cursor-pointer"
          >
        ...
        {/* Loading state */}
        {loading && (
          <div className="flex items-center justify-center py-12 text-ink-muted gap-2.5">
        ...
        {!loading && error && (
          <div className="py-12 text-center">
            <p className="text-sm font-normal text-ink-muted">{error}</p>
          </div>
        )}
          ...
          <>
            {featured && (
              <Link
                ...
                className="group relative grid md:grid-cols-2 rounded-3xl overflow-hidden border border-line bg-[#0B0D12] shadow-sm hover:shadow-xl transition-all duration-300 mb-10"
              >
                <div className="relative aspect-video md:aspect-auto md:min-h-72 overflow-hidden">
                  ...
                  <span className="absolute top-4 left-4 px-3 py-1 rounded-lg bg-accent text-white text-[11px] font-medium uppercase tracking-wider shadow-md">
                    {featured.category}
                  </span>
                </div>
                <div className="flex flex-col justify-center p-6 sm:p-10 space-y-3.5 bg-[#0B0D12] text-white">
                  <div className="flex items-center gap-3 text-[11px] font-normal text-neutral-400">
                    <span className="inline-flex items-center gap-1"><Calendar className="w-3.5 h-3.5 text-accent" /> {fmtDate(featured.publishedAt)}</span>
                    {featured.readingTime ? <span className="inline-flex items-center gap-1"><Clock className="w-3.5 h-3.5 text-accent" /> {featured.readingTime} min read</span> : null}
                  </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
              {gridPosts.map((post, idx) => (
                <Link
                  ...
                  to={`/blog/${post.slug}`}
                  className="group flex flex-col rounded-3xl bg-surface border border-line hover:border-accent/40 overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-lg animate-fade-up"
                ...
                >
                  ...
                  <div className="flex flex-col flex-1 p-5 space-y-2.5">
                    ...
                    <div className="flex items-center justify-between pt-2.5 border-t border-line">
                      ...
                      <span className="inline-flex items-center gap-1 text-[11px] font-medium text-accent group-hover:gap-1.5 transition-all">Read <ArrowRight className="w-3 h-3" /></span>
                    </div>
                  </div>
                </Link>
              ))}
