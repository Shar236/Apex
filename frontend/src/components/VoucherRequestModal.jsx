...
import {
  X, Clock, CheckCircle2, Loader2, LogIn, AlertCircle, ArrowRight, ShoppingCart, Ticket,
} from 'lucide-react';
 ...
 * inventory is currently empty. Backend-driven: it creates a real VoucherRequest
 * record — no localStorage, no mock state.
 *
 * Phases: confirm → (submitting) → success | duplicate | available | error
 */
export const VoucherRequestModal = () => {
  const navigate = useNavigate();
  const {
    voucherRequestProduct: product,
  ...
  // Reset whenever the modal (re)opens for a product.
  useEffect(() => {
  ...
  const goToRequests = useCallback(() => {
    closeVoucherRequest();
    navigate('/account?tab=voucher-requests');
  }, [closeVoucherRequest, navigate]);
  ...
  const submit = useCallback(async () => {
    ...
    setSubmitting(true);
    setError('');
    const res = await voucherRequestApi.submit(product._id || product.id);
    setSubmitting(false);
        ...
        <div className="px-6 py-6">
          {phase === 'confirm' && (
            <>
              ...
              <p className="text-sm font-normal text-ink-muted leading-relaxed">
                This voucher is temporarily unavailable, but you can request it from our team.
              </p>
              <p className="mt-3 text-sm font-medium text-ink bg-surface-raised border border-line rounded-2xl px-4 py-3 leading-relaxed">
                You will receive your voucher within <span className="text-accent font-semibold">1–2 hours</span> after
          ...
          {phase === 'success' && (
            <div className="text-center">
              ...
              <p className="mt-2 text-sm font-normal text-ink-muted leading-relaxed">
                Thank you for your request. Our team has received it and will process it as soon as possible.
              </p>
          ...
          {phase === 'duplicate' && (
            <div className="text-center">
              ...
              <p className="mt-2 text-sm font-normal text-ink-muted leading-relaxed">
                You already have a pending request for this voucher. Our team is processing it and you should receive it
                within <span className="text-accent font-semibold">1–2 hours</span>.
              </p>
              <div className="mt-6 flex flex-col-reverse sm:flex-row gap-2.5">
                <Button variant="ghost" size="md" fullWidth onClick={closeVoucherRequest}>
                  Close
          ...
          {phase === 'available' && (
            <div className="text-center">
              ...
              <p className="mt-2 text-sm font-normal text-ink-muted leading-relaxed">
                This voucher just became available. You can buy it right now and get instant delivery.
              </p>
              <div className="mt-6 flex flex-col-reverse sm:flex-row gap-2.5">
                <Button variant="ghost" size="md" fullWidth onClick={closeVoucherRequest}>
                  Not now
