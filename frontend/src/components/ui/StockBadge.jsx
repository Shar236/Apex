 ...
 *
 * NOTE: a product with zero available voucher codes is NEVER shown as
 * "Out of Stock" — it is a live product with temporarily empty inventory and
 * gets a neutral "Available on Request" pill instead.
 */
export default function StockBadge({ product, className = '' }) {
  ...
  const isRequestOnly =
    !isComingSoon && !isUnlimited &&
    (Number(avail) <= 0 ||
      String(product?.stockStatus || '').toUpperCase() === 'OUT OF STOCK' ||
  ...
  if (isRequestOnly) return <Badge tone="info" icon={<Ticket className="w-3 h-3" />} className={className}>Available on Request</Badge>;
