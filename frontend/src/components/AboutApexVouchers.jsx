  ...
  return (
    <section className="py-16 sm:py-24 bg-[var(--color-surface-raised)] border-b border-[var(--color-line)] transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center mb-16">
          ...
          <div className="lg:col-span-6 space-y-6">
            ...
            <h2 className="font-heading text-3xl sm:text-4xl lg:text-5xl font-light text-[var(--color-ink)] tracking-tight">
              ...
              <span className="text-[var(--color-accent)]">Test Smarter & Save More</span>
            </h2>

            <p className="text-[var(--color-ink-muted)] font-normal text-base leading-relaxed">
            ...
            <p className="text-[var(--color-ink-muted)] font-normal text-sm leading-relaxed">
          ...
          <div className="lg:col-span-6 flex justify-center">
            <div className="relative w-full max-w-lg bg-[var(--color-surface)] rounded-3xl p-6 shadow-xl border border-[var(--color-line)] overflow-hidden space-y-4">
              <div className="relative h-60 rounded-2xl overflow-hidden bg-[#0B0D12]">
                ...
                <div className="absolute bottom-4 left-4 right-4 text-white">
                  <span className="text-xs font-medium text-[var(--color-accent)] uppercase tracking-widest block">Apex Vouchers Desk</span>
                  <p className="font-heading font-medium text-lg">Trusted by Indian Study Abroad Aspirants</p>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-3 text-xs font-normal text-[var(--color-ink)]">
                <div className="p-3 rounded-xl bg-[var(--color-surface-raised)] border border-[var(--color-line)] flex items-center gap-2">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 pt-6 border-t border-[var(--color-line)]">
          {genuineStats.map((st, idx) => (
            <div key={idx} className="bg-[var(--color-surface)] rounded-2xl p-6 border border-[var(--color-line)] shadow-sm text-center space-y-1">
              <span className="font-heading font-normal text-3xl sm:text-4xl text-[var(--color-ink)] block">
                {st.number}
              </span>
              <span className="text-xs font-medium text-[var(--color-accent)] uppercase tracking-wider block">
                {st.label}
              </span>
              <span className="text-[11px] text-[var(--color-ink-muted)] font-normal block">
                {st.desc}
              </span>
            </div>
          ))}
