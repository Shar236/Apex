 ...
 *
 * SEO REWRITE NOTES:
 * - Added a "Quick answer" box + in-page TOC + two new H2 sections (CLB by
 *   program, score validity) to cover more of the "IELTS score for Canada"
 *   query cluster, not just the 8-7-7-7 headline stat.
 * - `title` / lede fallback strings are only used if the CMS SEO fields are
 *   empty. Set the real SEO title + meta description in the SEO Manager once
 *   it's back online rather than relying on these fallbacks long-term.
 * - New className `ca-toc` has no matching CSS yet in ielts-canada.css --
 *   style it (simple horizontal or wrapped pill list) or it renders as a
 *   bare bullet list.
 * - IELTS-to-CLB and PTE-Core-to-CLB figures verified against current IRCC
 *   equivalency data; the "CLB 9 = maximum points" line from the previous
 *   version was corrected (CLB 10 scores marginally higher).
 *
...
export default function IeltsCanada2026({ post = {}, relatedPosts = [] }) {
  const title =
    post.title || 'IELTS Score for Canada PR: The 8-7-7-7 Rule Explained (CLB 9 Guide, 2026)';
  ...
  return (
    <article className="blog-article blog-ielts-canada">
      <header className="ca-header">
        ...
        <p className="ca-lede">
          {post.excerpt ||
            'Wondering what IELTS score you need for Canada PR? The 8-7-7-7 rule is the IELTS General Training result — Listening 8.0, Reading 7.0, Writing 7.0, Speaking 7.0 — that converts to CLB 9, the benchmark that unlocks the biggest jump in Express Entry CRS points. Here&rsquo;s the full IELTS-to-CLB chart, the PTE Core equivalent, and the mistakes that quietly cost applicants points.'}
        </p>
      </header>
      ...
      <div className="ic-hero">
        <ArticleFigure
          ...
          alt="IELTS 8-7-7-7 score requirements for Canada Express Entry mapped to CLB 9"
          width={1200}
          height={630}
          priority
        />
      ...
      <div className="ca-body">
        <ArticleInfoBox variant="tip" title="Quick answer" icon={<Lightbulb className="w-4 h-4" />}>
          <p>
            The 8-7-7-7 rule means IELTS General Training bands of Listening 8.0, Reading 7.0,
            Writing 7.0 and Speaking 7.0 — the minimum in every skill to reach <strong>CLB 9</strong>,
            the Canadian Language Benchmark that maximizes your Express Entry language points. Your
            overall band score is not used: IRCC converts each of the four skills separately and
            takes your <strong>lowest</strong> result.
          </p>
        </ArticleInfoBox>
        ...
        <p>
          If you&rsquo;re immigrating to Canada through Express Entry — the Federal Skilled Worker
          Program, Canadian Experience Class, or a linked Provincial Nominee Program — reaching
          Canadian Language Benchmark (CLB) 9 is one of the single most rewarding moves for your CRS
          score. &ldquo;8-7-7-7&rdquo; is shorthand for the IELTS General Training band scores that
          map to CLB 9 in every ability:
        </p>
        <ScoreCards
          items={[
        ...
        <p>
          IRCC accepts <strong>IELTS General Training</strong>, not Academic, for permanent
          residence under Express Entry and for almost every Provincial Nominee Program stream.
          IELTS Academic is for study permits and university admissions only — book the wrong
          module and your result can&rsquo;t be used for your PR application at all.
        </p>

        <h2 id="ielts-clb-chart">IELTS to CLB Conversion Chart (2026)</h2>
        <p>
          IRCC doesn&rsquo;t read your overall IELTS band. Each of the four skills converts to a CLB
          level on its own, using the official equivalency table below.
        </p>
        <ComparisonTable
        ...
        <ArticleInfoBox
          variant="warning"
          title="Your CLB is your lowest skill — not the average"
          icon={<TriangleAlert className="w-4 h-4" />}
        >
          <p>
            IRCC converts each skill separately and takes the <strong>lowest</strong> of the four as
            your overall CLB. Score CLB 10 in Listening, Reading and Speaking but CLB 8 in Writing,
            and your application is assessed at CLB 8 — not an average of the four.
          </p>
        </ArticleInfoBox>

        <h2 id="why-clb9-matters">Why CLB 9 Matters for Your CRS Score</h2>
        <p>
          CLB 9 is the realistic ceiling most competitive applicants aim for. CLB 10 and above score
          only slightly higher in the core language factor, but typically requires bands of
          7.5&ndash;8.5 across all four skills — near-native territory few test-takers reach. Moving
          from CLB 8 to CLB 9 across all four skills lifts your core language points on its own, and
          combined with your education or foreign/Canadian work experience, CLB 9 can unlock{' '}
          <strong>up to 50 additional points</strong> under the skill-transferability factors —
          often enough to be the difference between an Invitation to Apply and another wait.
        </p>
        <ArticleFigure
        ...
        <p>
          Not every route needs CLB 9 — or even CLB 7. Check which minimum actually applies to your
          program before you over-prepare (or under-prepare):
        </p>
        <ComparisonTable
        ...
        <p>
          Since 2024, IRCC also accepts <strong>PTE Core</strong> for Canadian PR, and it follows the
          same lowest-skill rule as IELTS — there&rsquo;s no averaging here either. The approximate
          PTE Core range equivalent to CLB 9 is:
        </p>
        ...
        <p>
          Both IELTS and PTE Core results are valid for <strong>two years</strong> from your test
          date for any IRCC application, including an active Express Entry profile. If your result
          expires while you&rsquo;re still sitting in the pool, your profile can no longer be
          considered in a draw — so plan your test date around your expected Invitation to Apply,
          not just around when you submit your profile.
        </p>
        ...
        <ul>
          ...
          <li>Letting a result expire before PR is granted — see <a href="#score-validity">score validity</a> above.</li>
          <li>Not re-checking the CLB table, which IRCC updates periodically.</li>
          <li>
            Chasing CLB 9 in every skill when your actual program only needs CLB 5 or CLB 7 — or the
            reverse, assuming CLB 7 is competitive in a busy draw. See{' '}
